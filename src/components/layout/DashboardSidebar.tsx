...
import { Box, Typography, Button, Drawer, IconButton, useMediaQuery, useTheme } from '@mui/material';
import {
  ChevronLeft,
  ChevronRight,
  ChevronDown,
      ...
      <div className="p-4 border-b border-gray-200">
        ...
        <circle cx="301" cy="28" r="8" fill="#0279F2"/>
            ...
            <div className="w-8 h-8 bg-white rounded-lg flex items-center justify-center mx-auto">
              <svg width="30" height="20" viewBox="0 0 108 106" fill="none" xmlns="http://www.w3.org/2000/svg">
<path fill-rule="evenodd" clip-rule="evenodd" d="M87.3006 25.5C87.3671 25.2868 87.4336 25.0743 87.5 24.8623C88.6384 20.8014 88.3406 18.7631 85 15.8623L66 1.8623C61.0271 -1.00737 58.8635 -0.600929 56 3.3623L43.5 24.8623H6C2.68629 24.8623 0 27.5486 0 30.8623V99.8623C0 103.176 2.68629 105.862 6 105.862H57C61.4057 99.4154 56.1561 95.8321 45.3873 88.4814C39.7427 84.6284 32.5817 79.7403 24.5 72.8623C24.5 72.8623 23.8567 72.2839 23.5 71.8623C20.7252 68.5829 21.0542 65.2012 23.5 60.8623L43.1457 25.5H87.3006ZM58.9336 105.862H58.5C58.6427 105.883 58.7872 105.882 58.9336 105.862Z" fill="#0279F2"/>
<path d="M87.5 24.8623C87.4336 25.0743 87.3671 25.2868 87.3006 25.5C84.6983 33.8392 82.0368 43.0725 79.3996 52.2216C71.6681 79.0444 64.1453 105.143 58.9336 105.862H102C105.314 105.862 108 103.176 108 99.8623V30.8623C108 27.5486 105.314 24.8623 102 24.8623H87.5Z" fill="#014B99"/>
</svg>
        ...
        {(sidebarExpanded || isMobile) && (
          <>
            <button className="w-full text-left text-sm text-gray-600 hover:text-gray-900 py-2 flex items-center gap-2 hover:bg-gray-50 rounded-md px-2 transition-colors">
              <Download size={16} />
              {t('common.download') || 'Download Portfolio'}
            </button>
            <button className="w-full text-left text-sm text-gray-600 hover:text-gray-900 py-2 flex items-center gap-2 hover:bg-gray-50 rounded-md px-2 transition-colors">
              <Share2 size={16} />
              {t('common.share') || 'Share Profile'}
            </button>
          </>
        )}
      </div>
        ...
        <div className="px-4">
          ...
          <div className="mb-4">
            <h3 className="text-xs font-semibold text-gray-500 uppercase tracking-wider mb-2 px-2">
              {(sidebarExpanded || isMobile) ? t('common.dashboard').toUpperCase() : ''}
            </h3>
            ...
            >
              ...
              {(sidebarExpanded || isMobile) && <span>{t('common.overview')}</span>}
            </button>
          ...
          <div className="mt-8" />
          {(sidebarExpanded || isMobile) && <p className="text-sm text-gray-500 mb-4 ml-2 font-medium">PAGES</p>}
          ...
          >
            ...
            {(sidebarExpanded || isMobile) && <span className="font-medium">{t('profile.title')}</span>}
          </button>
          
          {openPages && (sidebarExpanded || isMobile) && (
          ...
          {userRole === 'employer' && (
            <>
              ...
              >
                ...
                {(sidebarExpanded || isMobile) && <span>{t('navigation.candidates')}</span>}
              </button>
            </>
          )}
      ...
      <div className="border-t border-gray-200 p-4">        
        ...
        >
          ...
          {(sidebarExpanded || isMobile) && <span>{t('common.help')}</span>}
        </button>
      </div>
    </div>
  );
  ...
  return (
    <>
      ...
      {isMobile ? (
        ...
        <div className={`${sidebarExpanded ? 'w-64' : 'w-20'} bg-white border-r border-gray-200 transition-all duration-300`}>
          {sidebarContent}
        </div>
      )}
