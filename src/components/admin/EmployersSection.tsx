...
export const EmployersSection: React.FC<EmployersSectionProps> = ({ darkMode }) => {
    const [analytics, setAnalytics] = useState<EmployerAnalyticsType | null>(null);
    const [employers, setEmployers] = useState<EmployerType[]>([]);
    const [loading, setLoading] = useState(true);
    const [error, setError] = useState<string | null>(null);
    const [selectedEmployer, setSelectedEmployer] = useState<EmployerType | null>(null);
    const [showKYCModal, setShowKYCModal] = useState(false);

    const fetchData = async () => {
        try {
            const [analyticsData, employersData] = await Promise.all([
                adminService.getEmployerAnalytics(),
            ...
            setError(e.message || 'Failed to load employer data');
        } finally {
            setLoading(false);
        }
    };
