...
// ... (Keep your Wallet Icon components: MetaMaskIcon, WalletConnectIcon, CoinbaseIcon here) ...
const MetaMaskIcon = () => (
  <svg width="24" height="24" viewBox="0 0 24 24" fill="none">
    <path d="M21.6 2L13.2 8.4L14.7 4.5L21.6 2Z" fill="#E17726" />
    <path d="M2.4 2L10.7 8.5L9.3 4.5L2.4 2Z" fill="#E27625" />
...
// 1. Create a NEW component that holds all your current logic
function IntegrationsContent() {
  ...
  // This hook is the reason for the build error if not suspended
  const searchParams = useSearchParams(); 
  const router = useRouter();
  const { user, isLoading: authLoading } = useAuth();
  ...
  // ... (Paste all your functions: formatAddress, handleConnectWallet, handleDisconnect, useEffects, initiateDigiLocker, fetchDocument here) ...
  // Be sure to include ALL the logic that was previously inside `IntegrationsPage`
  ...
  const initiateDigiLocker = async () => {
    try {
      ...
      const tokenData = await tokenResponse.json();
      if (!tokenResponse.ok || !tokenData.status) throw new Error(tokenData.error || 'Failed to generate token');
      ...
      const linkResponse = await fetch('/api/digilocker/generate-link', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ client_token: tokenData.client_token, redirect_url: redirectUrl })
      });

      const linkData = await linkResponse.json();
      if (!linkResponse.ok || !linkData.url) throw new Error(linkData.error || 'Failed to generate link');
      window.location.href = linkData.url;
    } catch (e: any) {
      console.error('DigiLocker initiation error:', e);
      alert(e.message || 'Unable to initiate DigiLocker.');
      setLoading(false);
    }
  };
  ...
  const fetchDocument = async () => {
    ...
    try {
      ...
      const response = await fetch('/api/digilocker/fetch-data', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ client_token: token, state: stateValue })
      });

      const data = await response.json();
      if (!response.ok || !data.success) throw new Error(data.error || 'Failed to retrieve documents');

      setDigilockerData(data.data);
      setNotice('Documents retrieved successfully!');
      ...
      if (userId) {
        try {
          ...
          await fetch('/api/digilocker/save-data', {
            ...
            body: JSON.stringify({ learner_id: userId, digilocker_data: data.data }),
          });
        } catch (saveError) { console.error(saveError); }
      }
    } catch (e: any) {
      console.error('Fetch document error:', e);
      alert(e.message || 'Unable to fetch documents.');
    } finally {
      setLoading(false);
    }
  };

  // Return your original JSX here (all the Box, Paper, Grid code...)
  return (
      <Box sx={{ p: 3 }}>
        <Paper elevation={1} sx={{ p: 4, borderRadius: 3 }}>
             {/* ... ALL YOUR ORIGINAL JSX CONTENT INSIDE THE PAPER ... */}
             {/* I am omitting the full JSX repetition to keep the answer clean, but paste EVERYTHING from your original return statement here, except the DashboardLayout wrapper */}
             
             {/* Example of what goes here: */}
             <Typography variant="h5" sx={{ fontWeight: 600, mb: 3 }}>Credentials</Typography>
             {/* ... Wallets Section ... */}
             {/* ... DigiLocker Section ... */}
             
             {/* IMPORTANT: Paste the FULL content that was inside DashboardLayout */}
             {/* The DigiLocker section, Wallets section, etc. */}
        </Paper>
      </Box>
  );
}

// 2. Export the Main Page Component which Wraps the Content in Suspense
export default function IntegrationsPage() {
  return (
    <DashboardLayout title="Integrations">
      <Suspense fallback={
